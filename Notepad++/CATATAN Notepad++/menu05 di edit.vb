...
'N =IF(WEEKDAY($M$5)=2,WEEKDAY(M5,2),WEEKDAY(M5))
    Range("O5").End(xlToLeft).Offset(0, 1).Select
'    Selection.Formula = "=WEEKDAY(RC[-1])"
'    Selection.Formula = "=IF(WEEKDAY(R5C" & LastCol & ")=2,WEEKDAY(RC[-1],2),WEEKDAY(RC[-1]))"
    
    Selection.Formula = "=IF(WEEKDAY(R5C" & LastCol & ")=2,WEEKDAY(RC[-1],1),WEEKDAY(RC[-1]))"
    ...
    '
'    Selection.Formula = "=IF(R5C" & LastCol & "=2,COUNTIF(R5C" & n_fty + 5 & ":R" & ntgl + 5 & "C" & n_fty + 5 & ",2)-1,COUNTIF(R5C" & n_fty + 5 & ":R" & ntgl + 5 & "C" & n_fty + 5 & ",2))"
        
    Selection.Formula = "=IF(R5C" & LastCol + 1 & "=2,COUNTIF(R5C" & n_fty + 5 & ":R" & ntgl + 5 & "C" & n_fty + 5 & ",2)-1,COUNTIF(R5C" & n_fty + 5 & ":R" & ntgl + 5 & "C" & n_fty + 5 & ",2))"
...
'O
    ...
    Selection.Formula = "=IF(RC[-1]=2,IF(R5C" & LastCol & "=2,COUNTIF(R5C" & n_fty + 5 & ":RC[-1],2)-1,COUNTIF(R5C" & n_fty + 5 & ":RC[-1],2)),0)"
    
    Selection.Formula = "=IF(RC[-1]=2,IF(R5C13=2,COUNTIF(R5C14:RC[-1],2)-1,COUNTIF(R5C14:RC[-1],2)),0)"
...
'    Selection.Formula = "=IF(RC[-1]>0,CONCATENATE(""WEEK"",RC[-1]),0)"
    ...
    Selection.Formula = "=IF(RC[-1]>0,CONCATENATE(""WEEK"",RC[-1]-1),0)"
...
'    Selection.Formula = "=IF(RC[-3]=1,WEEKNUM(RC[-4])-1,WEEKNUM(RC[-4]))"
    Selection.Formula = "=IF(WEEKDAY(R5C" & LastCol & ")=2,IF(RC[-3]=1,WEEKNUM(RC[-4],2)-1,WEEKNUM(RC[-4],2)),IF(RC[-3]=1,WEEKNUM(RC[-4])-1,WEEKNUM(RC[-4])))"
...
'    Selection.PasteSpecial Paste:=xlPasteValues
    ...
    ActiveSheet.Paste
        ...
        Range("Z5").End(xlToLeft).Offset(1, 2).Select
        Selection.Formula = "=IF(WEEKDAY(R5C13)=2,0,IF(RC[-1]>0,RC[-6]-RC[-1],0))"
